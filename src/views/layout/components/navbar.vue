        ...
        <el-row v-if="org">
          <el-col :span="12">
            <el-form-item label="企业logo">
              <el-upload
                ref="upload"
                :action="uploadServer+'/webapi/file/upload/boiler?type=logo'"
                name="picture"
...
import { changeOrgName } from "@/api/login";
  ...
  methods: {
    ...
    submitHomeForm() {
      this.$refs["homeForm"].validate(valid => {
        if (valid) {
          changeOrgName(this.$store.state.user.orgId,this.homeFormData.copyrightInfo1,1).then(response => {
          }).catch(reason => {
              this.$message.error(reason);
            });
